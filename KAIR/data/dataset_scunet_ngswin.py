...
class DatasetSyntheticTransfer(Dataset):
    """
    ADN Synthetic Metal Transfer Dataset for Fine-tuning
    
    Uses synthetic metal artifacts generated by ADN neural transfer (Phase 1):
    - Input: synthesized_metal_transfer/ (corrupted images)  
    - Ground truth: no_metal/ (clean images)
    
    Expected structure:
        <dataroot>/train/synthesized_metal_transfer/  (.npy files)
        <dataroot>/train/no_metal/                   (.npy files)
        <dataroot>/test/synthesized_metal_transfer/   (.npy files)
        <dataroot>/test/no_metal/                    (.npy files)
    """
    def __init__(self, opt):
        super().__init__()
        self.root = opt['dataroot_H']
        ...
        # Paths for synthetic transfer (corrupted) and clean images
        self.transfer_dir = os.path.join(self.root, self.phase, 'synthesized_metal_transfer')
        self.clean_dir = os.path.join(self.root, self.phase, 'no_metal')
        
        logging.info(f'Init {self.__class__.__name__} | phase={self.phase} | root={self.root}')
        logging.info(f'  Transfer dir: {self.transfer_dir}')
        logging.info(f'  Clean dir: {self.clean_dir}')
        
        # Find all .npy files in transfer directory
        self.transfer_files = []
        if os.path.exists(self.transfer_dir):
            for fn in sorted(os.listdir(self.transfer_dir)):
                if fn.endswith('.npy'):
                    self.transfer_files.append(os.path.join(self.transfer_dir, fn))
        
        assert self.transfer_files, f'No .npy files found in {self.transfer_dir}'
        
        # Match with clean files (same basenames)
        self.clean_files = []
        missing_clean = []
        
        for transfer_file in self.transfer_files:
            basename = os.path.basename(transfer_file)
            clean_file = os.path.join(self.clean_dir, basename)
            
            if os.path.exists(clean_file):
                self.clean_files.append(clean_file)
            else:
                missing_clean.append(basename)
                
        if missing_clean:
            logging.warning(f'Missing {len(missing_clean)} clean files. Examples: {missing_clean[:5]}')
            # Filter out transfer files without clean counterparts
            valid_pairs = []
            valid_clean = []
            for i, transfer_file in enumerate(self.transfer_files):
                basename = os.path.basename(transfer_file)
                clean_file = os.path.join(self.clean_dir, basename)
                if os.path.exists(clean_file):
                    valid_pairs.append(transfer_file)
                    valid_clean.append(clean_file)
            
            self.transfer_files = valid_pairs
            self.clean_files = valid_clean
        
        assert len(self.transfer_files) == len(self.clean_files), \
            f'Mismatch: {len(self.transfer_files)} transfer vs {len(self.clean_files)} clean files'
            
        logging.info(f'  ➜  {len(self.transfer_files)} valid pairs found')

    def __len__(self):
        return len(self.transfer_files)
    
    def __getitem__(self, idx):
        transfer_path = self.transfer_files[idx]
        clean_path = self.clean_files[idx]
        
        # Load numpy arrays
        L_np = np.load(transfer_path).astype(np.float32)  # Corrupted (input)
        H_np = np.load(clean_path).astype(np.float32)    # Clean (ground truth)
        
        # Handle different shapes
        if L_np.ndim == 3:
            L_np = L_np.squeeze()  # Remove singleton dimensions
        if H_np.ndim == 3:
            H_np = H_np.squeeze()
            
        # Ensure 2D
        if L_np.ndim != 2 or H_np.ndim != 2:
            raise ValueError(f'Expected 2D images, got L: {L_np.shape}, H: {H_np.shape}')
        
        # Normalize from HU values to [0,1] range
        # Typical CT range: [-1000, 3000] HU
        def normalize_ct(img):
            img = np.clip(img, -1000, 3000)  # Clip extreme values
            img = (img + 1000) / 4000.0      # Map [-1000,3000] to [0,1]
            return img
            
        L_np = normalize_ct(L_np)
        H_np = normalize_ct(H_np)
        
        # Convert to torch tensors with channel dimension
        L = torch.from_numpy(L_np).unsqueeze(0)  # Shape: [1, H, W]
        H = torch.from_numpy(H_np).unsqueeze(0)
        
        # Resize to training resolution (416x416)
        L = L.unsqueeze(0)  # [1, 1, H, W] for interpolation
        H = H.unsqueeze(0)
        L = F.interpolate(L, size=(416, 416), mode='bilinear', align_corners=False)
        H = F.interpolate(H, size=(416, 416), mode='bilinear', align_corners=False)
        L = L.squeeze(0)    # Back to [1, 416, 416]
        H = H.squeeze(0)
        
        return {
            'L': L,                # Corrupted input (synthetic metal artifacts)
            'H': H,                # Clean ground truth  
            'L_path': transfer_path,
            'H_path': clean_path
        }
    
    def __repr__(self):
        return f'{self.__class__.__name__}(n={len(self)}, phase="{self.phase}")'
