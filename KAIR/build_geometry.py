...
def imaging_geo(param):
    ...
    # Pick an implementation that works in the current environment.
    # Prefer ASTRA CUDA, fall back to ASTRA CPU, then ODL's generic backends.
    impl_order = ['astra_cuda', 'astra_cpu', 'skimage', 'numpy']
    last_err = None
    ray_trafo_hh = None
    for impl in impl_order:
        try:
            ray_trafo_hh = odl.tomo.RayTransform(reco_space_h, geometry_h, impl=impl)
            break
        except Exception as e:
            last_err = e
            continue
    if ray_trafo_hh is None:
        raise last_err if last_err is not None else RuntimeError('No suitable ODL tomo backend found')
